...
--   luarocks pack prisma-airs-intercept-postproxy 0.3.0-1        -> a .all.rock you can checksum and ship

package = "prisma-airs-intercept-postproxy"
version = "0.3.0-1"

source = {
  url = "git+https://github.com/PaloAltoNetworks/prisma-airs-integrations.git",
  tag = "prisma-airs-intercept-postproxy-v0.3.0",
}

description = {
  summary  = "Prisma AIRS inline scanning for Kong Gateway",
  detailed = [[
    Scans LLM prompts and completions through the Prisma AIRS sync scan API on
    both the request and the response leg. Fails closed by default. Runs at
    PRIORITY 760, below Kong's ai-proxy, so it inspects the OpenAI-normalised
    body the model actually receives. This lineage has no MCP path and refuses
    streaming responses; for either, use prisma-airs-intercept.
  ]],
...
build = {
  type = "builtin",
  modules = {
    ["kong.plugins.prisma-airs-intercept-postproxy.handler"] = "plugin/prisma-airs-intercept-postproxy/handler.lua",
    ["kong.plugins.prisma-airs-intercept-postproxy.schema"]  = "plugin/prisma-airs-intercept-postproxy/schema.lua",
  },
}
